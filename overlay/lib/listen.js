...
function initSfx() {
  if (goodRngSfx === undefined) {
    ...
    goodRngSfx = new Howl({src: ['MGS - Duel.mp3'], format: ["mp3"], volume: goodRngSfxConfig.volume, loop: goodRngSfxConfig.loop});
  }
  if (badRngSfx === undefined) {
    const badRngSfxConfig = config.get('sfx.bad_rng');
    badRngSfx = new Howl({src: ['MGS - Duel.mp3'], format: ["mp3"], volume: badRngSfxConfig.volume, loop: badRngSfxConfig.loop});
  }
}
