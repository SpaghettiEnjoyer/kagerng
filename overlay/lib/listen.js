...
function initSfx() {
  if (goodRngSfx === undefined) {
    ...
    goodRngSfx = (goodRngSfxConfig == undefined) ? new Howl({src: [""], volume: 1, loop: true}) : new Howl({src: [goodRngSfxConfig.data_url], volume: goodRngSfxConfig.volume, loop: goodRngSfxConfig.loop});
  }
  if (badRngSfx === undefined) {
    const badRngSfxConfig = config.get('sfx.bad_rng');
    badRngSfx = (badRngSfxConfig == undefined) ? new Howl({src: [""], volume: 1, loop: true}) : new Howl({src: [badRngSfxConfig.data_url], volume: badRngSfxConfig.volume, loop: badRngSfxConfig.loop});
  }
}
